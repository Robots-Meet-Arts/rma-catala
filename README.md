# Robots Meet Arts – Recursos en català

**Quan el pensament computacional es troba amb les humanitats**

Aquest repositori conté el conjunt de recursos en català del projecte Erasmus+ Robots Meet Arts.

---

## Sobre el projecte

Robots Meet Arts proposa integrar pràctiques de pensament computacional en els ensenyaments d'humanitats, adreçat als mestres de primària. Les activitats conviden els alumnes a imaginar, crear i expressar-se: els robots esdevenen personatges de relats, ballarins de coreografies, exploradors de civilitzacions antigues, missatgers entre les llengües.

---

## Contingut

**[Mòduls de formació](https://github.com/Robots-Meet-Arts/rma-catala/tree/main/1.Moduls_Formacio)**
L'itinerari complet de 6 mòduls adreçat als mestres de primària, que inclou els enfocaments pedagògics, les competències tècniques i les pràctiques inclusives.

**[Fitxes pedagògiques](https://github.com/Robots-Meet-Arts/rma-catala/tree/main/2.Plans_de_Curs_CA)**
29 activitats a punt per fer servir: 8 sessions introductòries i 21 sessions disciplinàries repartides en 7 disciplines d'humanitats.

**[Implementació inclusiva](https://github.com/Robots-Meet-Arts/rma-catala/tree/main/3.Inclusio)**
Recomanacions per adaptar les sessions de manera que tots els alumnes hi puguin participar plenament — tenint en compte el gènere, les diferències d'aprenentatge i la diversitat cultural.

**[Butlletins informatius](https://github.com/Robots-Meet-Arts/rma-catala/tree/main/4.Butlletins_Info)**
Novetats del projecte, notícies dels socis i materials de difusió.

**[Bones pràctiques](https://github.com/Robots-Meet-Arts/rma-catala/tree/main/5.Bones_Practiques)**
Informe de bones pràctiques basat en l'experiència i els resultats del projecte.

**[Llibre de curs complet](RMA_LessonBook_CAT.pdf)**
El llibre de curs complet que reuneix tots els continguts de formació en un sol document.

---

## Altres llengües

- English: [rma-english](https://github.com/Robots-Meet-Arts/rma-english)
- Français: [rma-francais](https://github.com/Robots-Meet-Arts/rma-francais)
- Nederlands (België): [rma-dutch](https://github.com/Robots-Meet-Arts/rma-dutch)
- Ελληνικά: [rma-ellinika](https://github.com/Robots-Meet-Arts/rma-ellinika)

---

## Socis del projecte

- L.A.B. – Laboratoire d'Aix-périmentation et de Bidouille (França)
- Universitat Rovira i Virgili (Espanya)
- UCLL (Bèlgica)
- STIMMULI for Social Change (Grècia)
- DSPI (Grècia)
- Maria Fortuny (Espanya)
- KS Leuven (Bèlgica)

---

## Llicència

Aquesta obra es posa a disposició d'acord amb els termes de la llicència Creative Commons Reconeixement – Compartir Igual 4.0 Internacional (CC BY-SA 4.0).

---

## Finançament

Cofinançat per la Unió Europea. Els punts de vista i les opinions expressats són únicament els dels seus autors i no reflecteixen necessàriament els de la Unió Europea ni els de l'EACEA.

Número de projecte: 2023-1-FR01-KA220-SCH-000151881
